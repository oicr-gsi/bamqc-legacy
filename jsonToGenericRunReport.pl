...
my %param = (
    ...
    table_columns =>
      { #### will show all possible columns, removing those that need to be explicitly indicated
        data => [
            qw/lane barcode groupid ext_name library insert_mean ins_stddev read_length raw_reads raw_yield mapped mismatch1 mismatch2 indel1 indel2 softclip1 softclip2 rpsp ontarget collapsed_yield collapsed_coverage/
        ],
        graph => [
            qw/lane barcode library read_breakdown insert_distr qual_hist qual_by_cycle mismatch_by_cycle indel_by_cycle softclip_by_cycle hardclip_by_cycle/
        ],
        tsv => [
            qw/lane barcode groupid ext_name library insert_mean ins_stddev read_length raw_reads raw_yield mapped mismatch1 mismatch2 indel1 indel2 softclip1 softclip2 rpsp ontarget collapsed_yield collapsed_coverage target_size num_targets target_file/
        ]
      },
...
if ( !exists $opt{r} ) {
    ...
    @{ $param{table_columns}{graph} } =
      grep( !/^insert_distr$/, @{ $param{table_columns}{graph} } );

}
if ( !exists $opt{H} ) {
    ### remove hard clipping info
    @{ $param{table_columns}{data} } =
      grep( !/^hardclip$/, @{ $param{table_columns}{data} } );
    @{ $param{table_columns}{graph} } =
      grep( !/^hardclip_by_cycle$/, @{ $param{table_columns}{graph} } );
}

#### get list of files
my @jsonFiles = @ARGV;
my %jsonHash  = GSI::bamqc::load_json(@jsonFiles)
  ; #### contains decoded json data. Hash keys are filename, values are json hashes
...
for my $run ( sort keys %runList ) {
    ...
    my @sorted_lanes =
      sort { $jsonHash{$a}{lane} <=> $jsonHash{$b}{lane} }
      grep { $jsonHash{$_}{"run name"} eq $run } keys %jsonHash;

    $html .=
      GSI::RunReport::write_tsv( \%param, \%jsonHash, \@sorted_lanes, $run );
    $html .=
      GSI::RunReport::data_table( \%param, \%jsonHash, \@sorted_lanes, $run )
      ;    ### parameters, jsonHash, jsonfiles - in order, id
    $html .=
      GSI::RunReport::coverage_table( \%param, \%jsonHash, \@sorted_lanes,
        $run )
      if ( $param{showCoverageTable} );
    $html .=
      GSI::RunReport::graph_table( \%param, \%jsonHash, \@sorted_lanes, $run )
      if ( $param{showGraphTable} && $param{plotData} )
      ;    ### set this as a parameter than can be turned off, on by default
    $html .=
      GSI::RunReport::lane_info( \%param, \%jsonHash, \@sorted_lanes, $run )
      if ( $param{showLaneInfo} );
...
sub usage {
    ...
    print
"\t-p print all images.  Default is to only show thumbnails (with links).\n";
