...
sub readmap_piechart{
	...
	$pieChart{labels} = ["on target", "off target", "repeat/low quality", "unmapped"];
		...
		if ((scalar @tempArray) > 0)   ### do not proceed if no keys
		{
			...
			if ($histMax > 0)
			{
				for (my $i = 0; $i <= $histMax; $i++)
				{
					...
					my $colour_code = $i < $$p{qualcut}{low} ? "bad" : $i < $$p{qualcut}{high} ? "mid" : "good";
					push (@{$barChart{colours}}, $$p{colours}{$colour_code});
				}
			}
		}
	}
	barGraph($path, "qualHist.png", $barChart{values},$barChart{labels},$barChart{colours}, "$title Quality Histogram", "Base Quality", "# Bases", "");
}
...
sub noncollapsed_base_coverage{
	...
	if (defined $$jHash{"non collapsed bases covered"})
	{
		...
		barGraph($path, "nonCollapsedCover.png", $barChart{values},$barChart{labels},$barChart{colours}, "$title Non Collapsed Coverage", "Coverage Depth", "% Target Covered", ", ylim=c(0, 100)");
		return 1;
	}else{
		return 0;
	}
...
sub collapsed_base_coverage{
	...
	if (defined $$jHash{"collapsed bases covered"})
	{
		...
		barGraph($path, "collapsedCover.png", \@values, \@labels, \@colours, "$title Collapsed Coverage", "Coverage Depth", "% Target Covered", ", ylim=c(0, 100)");
		return 1;
	}else{
		return 0;
	}
...
sub readlength_histogram{
	...
	lineGraph($path, "readLength.png", \@lineX, \@lineY, \@colours, "$title Read Length Histogram", "Read Length (bp)", "Number of reads", "");
	return 1;
}
...
sub insert_graph{
	...
	lineGraph($path, "insert.png", \@lineX, \@lineY, \@colours, "$title Insert Distribution", "Insert Size (bp)", "Pairs", "");
	return 1;
}
...
sub quality_by_cycle{
			...
			for my $cyc (sort {$a <=> $b} keys %lineHash)
			{
				...
				else
				{
					if ($r eq "read 1")
					{
						push(@lineX, $cyc);
						push(@lineY, $qual);
						$read1max++;  ### increment the read max, this will determine cycle for read 2
					}
					elsif ($r eq "read 2")
					{
				...
				my $colour_key=	$qual < $$p{qualcut}{low} ? "bad" :
	...
	lineGraph($path, "qualCycle.png", \@lineX, \@lineY, \@colours, "$title Quality by Cycle", "Cycle", "Mean Quality", ", ylim=c(0, $$p{qualLineMax})");
	return 1;
}
...
sub mismatch_by_cycle{
	...
	for my $r (@{$$p{reads}})
	{
		...
		if ((scalar keys %{ $$jHash{"$r mismatch by cycle"} }) > 0)
		{
			...
			for my $cyc (sort {$a <=> $b} keys %errorHash)
			{
				...
				{
					push(@lineY, (($errorHash{$cyc} / ($alignedHash{$r}{$cyc} + $insertHash{$r}{$cyc})) * 100));
				}
				else
				{
					push(@lineY, 0);
	...
	lineGraph($path , "misCycle.png", \@lineX, \@lineY, \@colours, "$title Mismatches by Cycle", "Cycle", "% Bases Mismatched", ", ylim=c(0, 10)");
	return 1;
}
...
sub indel_by_cycle{
	...
	lineGraph($path, "indelCycle.png", \@lineX, \@lineY, \@colours, "$title Indels by Cycle", "Cycle", "% Bases Inserted/Deleted", ", ylim=c(0, 10)");
	return 1;
}
...
sub softclip_by_cycle{
	...
	lineGraph($path, "softCycle.png", \@lineX, \@lineY, \@colours, "$title Soft Clips by Cycle", "Cycle", "% Bases Soft Clipped", ", ylim=c(0, 100)");
	return 1;
}
...
sub hardclip_by_cycle{
	...
	lineGraph($path, "hardCycle.png", \@lineX, \@lineY, \@colours, "$title Hard Clips by Cycle", "Cycle", "% Bases Hard Clipped", ", ylim=c(0, 100)");
	return 1;
}
